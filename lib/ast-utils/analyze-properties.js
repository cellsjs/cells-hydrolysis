        ...
        if (property.value.type == 'ObjectExpression') {
            ...
            for (var j = 0; j < propDescExpr.properties.length; j++) {
                ...
                switch (propertyKey) {
                    ...
                    case 'computed':
                    {
                        prop.computed = astValue.expressionToValue(propertyArg.value);
                        prop.computedNode = propertyArg.value;
                        if (prop.computed === undefined) prop.computed = astValue.CANT_CONVERT;
                    }
                      break;
                    case 'readOnly':
                        {
